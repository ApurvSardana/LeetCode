...
29                dp[i][j] = max({dp[i-1][j], dp[i][j-1], dp[i-1][j-1], dp[i-1][j-1] + nums1[i-1]*nums2[j-1]});
...
40        // cout << endl;
41
42        if((all_neg1 && all_pos2) || (all_neg2 && all_pos1)){
43            return -mn1*mn2;
44        }
45        return ans;
46    }
47
48    int maxDotProduct(vector<int>& nums1, vector<int>& nums2) {
49        return f(nums1,nums2);
50        // return max(f(nums1, nums2), f(nums2, nums1));
51    }
52};